	...
	<div class="content  gradient-custom-1 overflow-auto">
		...
		<div class="cards">
			...
			try {
				...
				List<ViewBlog> list = blogDao.getAllBlogs();

				for (ViewBlog blog : list) {
			%>
			<div class="card border-dark mb-3 scroll" style="max-width: 25rem;">
				<%
				session.setAttribute("blogPostId", blog.getId());
				%>
				...
				<div class="card-footer">
					<a class="" data-toggle="collapse" href="#blogcom<%=blog.getId()%>"
						...
						aria-controls="collapseExample"> Add Comments </a>
				</div>
				<div class="card-footer">
					<a class="" data-toggle="collapse"
						href="#collapseExam<%=blog.getId()%>" role="button"
						aria-expanded="false" aria-controls="collapseExample"> View
						Comments </a>
				</div>
				<div class="collapse toggle" id="blogcom<%=blog.getId()%>">
					<div class="card card-body" id="formbody">
						<form
							action="AddCommentServlet?blogPostId=<%=session.getAttribute("blogPostId")%>"
							method="post" class="form-inline my-2 my-lg-0">
							<input class="form-control mr-sm-2" type="text"
								placeholder="Add comments" name="comment" aria-label="Search">
				...
				%>
				<div class="collapse toggle" id="collapseExam<%=blog.getId()%>">
